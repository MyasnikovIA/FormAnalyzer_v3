# ЗАПРОС К LLM: АНАЛИЗ ФОРМЫ Forms/HospitPlanning/modals/graphics_add_edit.frm
...
**Анализируемая форма:** Forms/HospitPlanning/modals/graphics_add_edit.frm
**Базовая форма:** C:\AppServ\www\5_mis_MEDDEV-151210\Forms\HospitPlanning\modals\graphics_add_edit.frm
...
**Дата генерации:** Tue May 19 13:12:13 GMT+07:00 2026
...
**Имя компонента:** scheduleUpgrade
**Источник:** Forms/HospitPlanning/modals/graphics_add_edit.frm
**Базовая форма:** C:\AppServ\www\5_mis_MEDDEV-151210\Forms\HospitPlanning\modals\graphics_add_edit.frm

**SQL код:**

```xml
<cmpAction name="scheduleUpgrade" compile="true">
        <![CDATA[
        declare
          nIS_PRIORITY         NUMBER;
          dDATA_CREATE         DATE;
        ...
        begin
          if :pnIS_ACTIVE = 1 then
            select max(s.IS_PRIORITY)
              into nIS_PRIORITY
              from D_V_HPK_SCHEDULE s
             where s.HOSP_PLAN_KINDS = :pnHOSP_PLAN_KINDS
               and s.IS_ACTIVE = 1
             @if(:pnID){
               and s.ID != :pnID
             @}
               and D_PKG_DAT_TOOLS.DATE_RANGES_ERROR(to_date(:pdDATE_BEGIN, 'DD.MM.YYYY'), to_date(:pdDATE_END, 'DD.MM.YYYY'), s.DATE_BEGIN, s.DATE_END, 1) > 0;

            if nIS_PRIORITY is not null then
              if nIS_PRIORITY = 0 and :pnIS_PRIORITY = 0 then
                D_P_EXC('На указанный интервал уже есть действующий график. Установите дополнительно признак "Приоритетный" если требуется создать замещающий график.');
              end if;

              if nIS_PRIORITY = 1 and :pnIS_PRIORITY = 1 then
                D_P_EXC('На указанный интервал уже есть приоритетный график');
              end if;

              if nIS_PRIORITY = 1 and :pnIS_PRIORITY = 0 then
                D_P_EXC('На указанный интервал уже есть действующий график.');
              end if;
            end if;
          end if;

          if :pnID is not null then
            select hs.DATE_CREATE
              into dDATA_CREATE
              from D_V_HPK_SCHEDULE hs
             where hs.ID = :pnID;
          else
            dDATA_CREATE := sysdate;
          end if;

          D_PKG_HPK_SCHEDULE.UPG(pnID              => :pnID,
                                 pnLPU             => :pnLPU,
                                 pnHOSP_PLAN_KINDS => :pnHOSP_PLAN_KINDS,
                                 pnIS_ACTIVE       => :pnIS_ACTIVE,
                                 pnIS_PRIORITY     => :pnIS_PRIORITY,
                                 pnSCHEDULE        => :pnSCHEDULE,
                                 pdDATE_BEGIN      => :pdDATE_BEGIN,
                                 pdDATE_END        => :pdDATE_END,
                                 pnOVER_LIMITS     => :pnOVER_LIMITS,
                                 pdDATE_CREATE     => dDATA_CREATE);
        end;
        ]]>
        <cmpActionVar name="pnLPU" src="LPU" srctype="session" />
        <cmpActionVar name="pnID" src="ID" srctype="var" />
        <cmpActionVar name="pnHOSP_PLAN_KINDS" src="HOSP_PLAN_KINDS" srctype="var" />
        <cmpActionVar name="pnIS_ACTIVE" src="cbActive" srctype="ctrl" />
        <cmpActionVar name="pnIS_PRIORITY" src="cbPriority" srctype="ctrl" />
        <cmpActionVar name="pnSCHEDULE" src="ueSchedule" srctype="ctrl" />
        <cmpActionVar name="pdDATE_BEGIN" src="deDateBegin" srctype="ctrl" />
        <cmpActionVar name="pdDATE_END" src="deDateEnd" srctype="ctrl" />
        <cmpActionVar name="pnOVER_LIMITS" src="cbOverLimits" srctype="ctrl" />
    </cmpAction>
```

**Используемые таблицы/вьюхи:** D_V_HPK_SCHEDULE
**Используемые пакеты/функции:** D_PKG_DAT_TOOLS.DATE_RANGES_ERROR, D_PKG_HPK_SCHEDULE.UPG
