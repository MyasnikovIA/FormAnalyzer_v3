...
**Дата генерации:** Tue May 19 08:06:05 GMT+07:00 2026
...
### Вьюха №1: D_V_HPK_PLANS

**Используется в формах:**
- Forms/HospPlan/hospplanperiod_edit.frm
...
**Статистика:**
- Всего вьюх с таблицами: 1
- Всего уникальных таблиц: 2
...
begin
  ...
  -- Инициализация бизнес-процесса --
  D_PKG_BPENV.BEFOREBP(pnLPU,null,nCID,null,'HPK_PLANS_UPDATE',pnID);
  CHECKS(pnID,
         null,
         pdPLAN_DATE,
         pnMALE_COUNT,
         ...
         pnGEN_COUNT);
  begin
    update D_HPK_PLANS t set
      t.PLAN_DATE                   = trunc(pdPLAN_DATE),
      t.MALE_COUNT                  = pnMALE_COUNT,
      t.OPER_COUNT                  = pnOPER_COUNT,
      t.GEN_COUNT                   = pnGEN_COUNT
     where t.ID      = pnID
       and t.LPU     = pnLPU;
...
```sql
CREATE OR REPLACE PROCEDURE d_pkg_hpk_plans.upd(IN pnid numeric, IN pnlpu numeric, IN pdplan_date timestamp without time zone, IN pnmale_count numeric, IN pnoper_count numeric, IN pngen_count numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $procedure$
DECLARE
...
BEGIN
    ...
    CALL d_pkg_bpenv.beforebp(pnlpu, (null)::numeric, ncid, (null)::numeric, 'HPK_PLANS_UPDATE', pnid);
    CALL d_pkg_hpk_plans.checks(pnid, (null)::numeric, pdplan_date, pnmale_count, pnoper_count, pngen_count);
    BEGIN
        update d_hpk_plans t set plan_date = trunc(pdplan_date) , male_count = pnmale_count , oper_count = pnoper_count , gen_count = pngen_count where t.id = pnid::bigint
    ...
    END IF;
    CALL d_pkg_bpenv.afterbp(pnlpu, (null)::numeric, ncid, (null)::numeric, 'HPK_PLANS_UPDATE', pnid);
END
$procedure$
```
