# ЗАПРОС К LLM: АНАЛИЗ ФОРМЫ Forms/HospitPlanning/select_bed_type_by_dep.frm
...
**Анализируемая форма:** Forms/HospitPlanning/select_bed_type_by_dep.frm
**Базовая форма:** C:\AppServ\www\5_mis_MEDDEV-151210\Forms\HospitPlanning\select_bed_type_by_dep.frm
...
**Дата генерации:** Tue May 19 13:13:18 GMT+07:00 2026
...
**Имя компонента:** DS_BED_TYPES
**Источник:** Forms/HospitPlanning/select_bed_type_by_dep.frm
**Базовая форма:** C:\AppServ\www\5_mis_MEDDEV-151210\Forms\HospitPlanning\select_bed_type_by_dep.frm

**SQL код:**

```xml
<component cmptype="DataSet" name="DS_BED_TYPES" mode="Range">
	<![CDATA[
	 	select t.ID,
		       t.BT_CODE,
		       t.BT_NAME
		 ...
		 where t.ID in
			(select t1.BED_TYPE_ID
			   from d_v_beds_fund t1
			  where t1.PID = :DEP_ID
			    and trunc(sysdate) >= t1.BEGIN_DATE
			    and (t1.END_DATE is null or trunc(sysdate) <= t1.END_DATE))
	]]>
	<component cmptype="Variable" name="DEP_ID" src="DEP_ID" srctype="var" get="v1" />
	<component cmptype="Variable" type="count" src="r1c" srctype="var" default="15" />
	<component cmptype="Variable" type="start" src="r1s" srctype="var" default="1" />
</component>
```

**Используемые таблицы/вьюхи:** D_V_BED_TYPES, D_V_BEDS_FUND
